...
- **低开销**：用户态缓冲 + 无锁 + 无阻塞热路径，显著降低 CPU 使用率与 GC 压力。生成环境单进程一小时写入30g以上日志，单机写入1T以上日志。
...
| `LogInfoBeforeFileSizeBytes` | `int` | 日志文件达到多少大小前输出Info 级别日志（-1 表示不限制） |
