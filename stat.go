...
type additionMsgStatReportFunc func(key string, value *MsgStatData, avgProcessTime time.Duration, avgSuccProcessTime time.Duration)

type ReportStatData struct {
...
type MsgStat struct {
	...
	statChan          chan *ReportStatData
	flushChan         chan struct{}
	flushWait         sync.WaitGroup
	additionMsgReport additionMsgStatReportFunc
}
...
func GetDefaultMsgStat() (*MsgStat, bool) {
	return defaultMsgStat, defaultMsgStat != nil
}

func InitDefaultMsgStat(srvName string) {
	if defaultMsgStat == nil {
		defaultMsgStat = NewMsgStat(srvName, noAdditionMsgStatReport)
	}
}

func noAdditionMsgStatReport(key string, value *MsgStatData, avgProcessTime time.Duration, avgSuccProcessTime time.Duration) {
}

func NewMsgStat(svrName string, additionMsgReport additionMsgStatReportFunc) *MsgStat {
	m := &MsgStat{additionMsgReport: additionMsgReport}

	cfgLoader := MustDefaultCfgLoader()
	var err error
	m.FileLogger, err = InitFileLogger(
	...
	m.statChan = make(chan *ReportStatData, 65536)
	m.flushChan = make(chan struct{})
...
func FlushStat() {
	...
	defaultMsgStat.flushWait.Add(1)
	defaultMsgStat.flushChan <- struct{}{}
	defaultMsgStat.flushWait.Wait()
}

func SetAdditionMsgStatReport(reportFunc additionMsgStatReportFunc) {
	mustDefaultMsgStat().SetAdditionMsgReport(reportFunc)
}

func (m *MsgStat) SetAdditionMsgReport(reportFunc additionMsgStatReportFunc) {
...
func (m *MsgStat) RunStat() {
	...
	for {
		...
		case <-m.flushChan:
			...
			m.PrintAllStat()
			lastPrintTime = now
			_ = m.FileLogger.Flush()
			m.flushWait.Done()
		case <-ticker.C:
			now := time.Now()
			if now.Sub(lastPrintTime) >= Interval {
				m.PrintAllStat()
				lastPrintTime = now
			}
...
func (m *MsgStat) SetStat(data *ReportStatData) {
	// 获取统计结点，不存在则插入
	key := data.key
	if _, ok := m.statData[key]; !ok {
		m.statData[key] = &MsgStatData{}
...
func (m *MsgStat) ReportStat(key string, t int, result int, processTime time.Duration) {
	data := &ReportStatData{key: key, reportType: t, result: result, processTime: processTime}
	select {
